...
public sealed class GroupsController(IGroupService groups, IEntraGroupSyncService entraGroupSync) : ApiControllerBase
{
    ...
    /// <response code="409">A group with that name already exists.</response>
    [HttpPost]
    ...
    /// <response code="409">The user is already a member.</response>
    ...
    /// Idempotent.
    /// </summary>
    /// <response code="400">The group has no <c>entraGroupId</c>, or Entra sync is disabled on this host (<c>Entra:Enabled</c> is false).</response>
    /// <response code="404">No such group.</response>
    ...
    /// </summary>
    /// <response code="400">Entra sync is disabled on this host and at least one group is linked.</response>
    [HttpPost("sync-entra")]
