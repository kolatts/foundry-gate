...
public class UserDetailPageTests : WebTestContext
{
    ...
    {
        Api.UserResult = ApiCallResult<UserDetailResponse>.Ok(detail);
    ...
    {
        var page = RenderDetail(WebTestData.UserDetail(WebTestData.User(displayName: "Dev Eloper", email: "dev@example.test")));

        page.WaitForAssertion(() =>
        {
            Assert.Equal("Dev Eloper", page.Find("[data-testid=user-name]").TextContent.Trim());
            Assert.Equal("dev@example.test", page.Find("[data-testid=user-email]").TextContent.Trim());
    ...
    public void Shows_the_resolved_level_and_tier_for_the_current_allocation()
    {
        var page = RenderDetail(WebTestData.UserDetail(
            allocation: WebTestData.Allocation(allocatedTokens: 20_000_000, level: QuotaLevelType.GroupMax)));
    ...
    [Fact]
    public void Switching_to_unlimited_and_saving_writes_a_tier_not_a_number()
    {
        // The load-bearing claim of this wave: a budget IS a tier (D-013), so the request carries
        // IsUnlimited or exactly one configured cap and never a free-form figure.
        var page = RenderDetail(WebTestData.UserDetail(WebTestData.User(userId: 42, isUnlimited: false, monthlyTokenQuota: 5_000_000)));
        page.WaitForAssertion(() => Assert.NotNull(page.Find("[data-testid=user-quota-save]")));

        page.Find("input[data-testid=user-quota-unlimited]").Change(true);
        page.WaitForAssertion(() => Assert.False(page.Find("[data-testid=user-quota-save]").HasAttribute("disabled")));
        page.Find("[data-testid=user-quota-save]").Click();

        page.WaitForAssertion(() =>
        {
            var (userId, request) = Assert.Single(Api.QuotaUpdates);
            Assert.Equal(42, userId);
            Assert.True(request.IsUnlimited);

            // Unlimited wins over any cap, so the stale number must not travel with it.
            Assert.Null(request.MonthlyTokenQuota);
        });
    }

    [Fact]
    public void Saving_is_disabled_until_the_budget_actually_changes()
    {
        var page = RenderDetail(WebTestData.UserDetail(WebTestData.User(monthlyTokenQuota: 5_000_000)));

        page.WaitForAssertion(() => Assert.True(page.Find("[data-testid=user-quota-save]").HasAttribute("disabled")));
        Assert.Empty(Api.QuotaUpdates);
    }
