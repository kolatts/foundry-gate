...
public class MePageTests : WebTestContext
{
    ...
    {
        Api.MeResult = Ok(WebTestData.Profile(
            quota: WebTestData.Allocation(resolvedLevelType: QuotaLevelType.GroupMax)));
    ...
    [Fact]
    public void The_cli_snippets_match_the_verified_instructions_in_cli_setup_mdx()
    {
        var page = RenderPage<Me>();
        var markup = page.Markup;

        // CLAUDE.md invariant 3: these are the empirically verified settings, copied from
        // docs-site/.../getting-started/cli-setup.mdx. If the doc changes, this test changes with it.
        Assert.Contains("CLAUDE_CODE_USE_FOUNDRY=1", markup, StringComparison.Ordinal);
        Assert.Contains("ANTHROPIC_FOUNDRY_BASE_URL=https://ai.example.test/anthropic", markup, StringComparison.Ordinal);
        Assert.Contains("""env_http_headers = { "api-key" = "FOUNDRYGATE_API_KEY" }""", markup, StringComparison.Ordinal);
        Assert.Contains("""wire_api = "responses" """.TrimEnd(), markup, StringComparison.Ordinal);
        Assert.Contains("x-api-key", markup, StringComparison.Ordinal);
    }
