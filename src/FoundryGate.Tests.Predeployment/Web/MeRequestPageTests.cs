...
public class MeRequestPageTests : WebTestContext
{
    ...
    [Fact]
    public void A_fork_with_one_option_left_submits_that_tiers_cap_not_unlimited()
    {
        // The reviewer's probe: a two-tier fork (Standard + Power, no Unlimited product). Standard
        // leaves exactly one option, and the shortcut that pre-selects it has to go through the
        // same handler a click does — setting only the product id leaves RequestedQuota null, which
        // SubmitQuotaIncreaseRequest documents as "requesting unlimited". D-013's whole point is
        // that the number and the tier are the same thing.
        Api.QuotaTiersResult = ApiCallResult<IReadOnlyList<QuotaTierResponse>>.Ok(
        [
        ...
        Assert.Equal(20_000_000, Assert.Single(Api.SubmittedRequests).RequestedQuota);
    }
    ...
    [Fact]
    public void Asks_the_api_only_for_the_callers_own_pending_requests()
    {
        Api.MeResult = ApiCallResult<UserProfileResponse>.Ok(WebTestData.Profile(userId: 314));
    ...
    [Fact]
    public void An_admin_is_not_told_a_strangers_pending_request_is_their_own()
    {
        // GET /requests hands an admin every user's requests. Without the UserId filter, any admin
        // opening this page was told "you already have a request waiting for review" — and the form
        // stayed locked for as long as anyone in the fork had something pending.
        SignInAsAdmin();
        Api.MeResult = ApiCallResult<UserProfileResponse>.Ok(WebTestData.Profile(userId: 1, displayName: "Ada Admin"));
    ...
    {
        Api.MeResult = ApiCallResult<UserProfileResponse>.Ok(
    ...
    public void A_409_warns_and_locks_the_form_without_navigating_away()
    {
        ...
        // Locked, but without inventing a submission date the page was never told.
        var alert = page.Find("[data-testid='request-pending']").TextContent;
        Assert.Contains("waiting for review", alert, StringComparison.Ordinal);
        Assert.DoesNotContain("submitted", alert, StringComparison.Ordinal);
        Assert.True(page.Find("[data-testid='request-submit']").HasAttribute("disabled"));
    }
    ...
    private static void SelectTier(IRenderedComponent<IComponent> page, string productId)
    {
    ...
    private static void Type(IRenderedComponent<IComponent> page, string justification) =>
