...
| `Security__RateLimits__Reveal__PermitLimit` / `__WindowSeconds` | env var | The `POST /keys/me/reveal` cap, per caller `oid`. Defaults to **5 per 60 seconds** — the values that shipped as constants (#181). Validated at startup (`1–10000` permits, `1–3600` seconds) |
| `Security__RateLimits__Rotate__PermitLimit` / `__WindowSeconds` | env var | The `POST /keys/me/rotate` cap. Defaults to **3 per 60 seconds** — tighter than reveal, because a rotation is a write the gateway feels and breaks whatever the developer has configured |
| `Security__RevealAnomaly__Threshold` / `__WindowMinutes` | env var | The reveal anomaly signal (#180): the reveal that pushes one key past **10** reveals inside a rolling **60** minutes logs a Warning and writes a `key.reveal-anomaly` audit row, once per window. The request still succeeds — a limiter catches a fast drain, this catches a patient one. There is no on/off switch; set the threshold unreachably high to silence it |
