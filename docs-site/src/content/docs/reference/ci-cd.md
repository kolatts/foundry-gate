...
| `_postdeployment-tests.yml` | waits for `/health`, really runs `dotnet test` on `FoundryGate.Tests.Postdeployment` with `FG_API_BASE_URL` / `FG_UI_BASE_URL`, and reports the step’s actual outcome; the step is `continue-on-error` so a failure does not gate the chain until [#139](https://github.com/kolatts/foundry-gate/issues/139). The project holds the `/api/v1` 401 contract ([#102](https://github.com/kolatts/foundry-gate/issues/102)) plus the scaffold smoke test; the 403/200 half needs a delegated Entra token and reports as *skipped* in CI, and the summary says so — a green line here is not evidence the stack works | `environment` | `summary` |
...
| 6 | `Postdeployment tests` | the unauthenticated 401 contract plus the scaffold smoke test — read-only in CI, because the two tests that write (`GET /users/me` self-provisioning) need a delegated token CI cannot mint and skip without one |
